import { Form, Select, SelectItem, SelectSkeleton, ComboBox, Button, TextInput, FormGroup, RadioButtonGroup, RadioButton, RadioButtonValue, ToastNotification, NumberInput } from 'carbon-components-react';
import React, { ChangeEvent, useEffect, useMemo, useRef, useState } from 'react';
import useTranslation from '../../core/utils/translation';
import Save24 from '@carbon/icons-react/lib/save/24';
import Undo24 from '@carbon/icons-react/lib/undo/24';
import { Formik, FormikProps, FormikValues } from 'formik';
import { DISPENSING_UNITS_CONCEPT_ID, STOCK_ITEM_CATEGORY_CONCEPT_ID } from '../../config';
import { useLazyGetConceptByIdQuery, useLazyGetConceptsQuery, useLazyGetDrugsQuery, ConceptFilterCriteria, DrugFilterCriteria } from '../../core/api/lookups'
...

                        {
                            ...
                            <Select invalid={touched.dispensingUnitPackagingUoMUuid && !!errors.dispensingUnitPackagingUoMUuid} invalidText={t2(errors.dispensingUnitPackagingUoMUuid)}
                                ...
                                {packagingUnits?.map(uom => {
                                    return <SelectItem key={uom.uuid} value={uom.uuid} text={`${uom?.packagingUomName ? `${uom.packagingUomName} ${uom?.factor ?` - ${uom?.factor}`: ""}` : ""}`} />
                                })}
                            </Select>
                        }
                        {
