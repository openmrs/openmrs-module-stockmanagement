...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(sessionApi.endpoints.logout.matchPending, (state, action) => {

      })
      .addMatcher(sessionApi.endpoints.logout.matchFulfilled, (state, action) => {
        state.user = null;
        state.isAuthenticated = false;
        state.authenticationMessage = null;
        state.status = LoadingStatus.IDLE;
      })
      .addMatcher(sessionApi.endpoints.logout.matchRejected, (state, action) => {
        state.user = null;
        state.isAuthenticated = false;
        state.authenticationMessage = null;
        state.status = LoadingStatus.IDLE;
      ...
      .addMatcher(sessionApi.endpoints.getSession.matchFulfilled, (state, action) => {
        if (action.payload.authenticated) {
          if (process.env.NODE_ENV === "development") {
            setCookie("JSESSIONID", action.payload.sessionId, null);
          }
      ...
      .addMatcher(sessionApi.endpoints.getStockManagementSession.matchFulfilled, (state, action) => {
...
export const selectPrivileges = (state: RootState) => state.auth?.user?.privileges;
export const selectPrivilegeScopes = (state: RootState) => state.auth?.privilegeScopes;