import React, { useMemo, CSSProperties, useState, ChangeEvent, useRef, useCallback } from 'react';
...
import { URL_LOCATIONS, URL_LOCATIONS_EDIT, URL_LOCATIONS_NEW } from '../../config';
  ...
  const rows: Array<any> = useMemo(() =>
    locations?.results?.filter(p => {
      ...
      actions: canCreateLocation ? <div key={`${location?.uuid}-actions`} style={{ display: "inline-block", whiteSpace: "nowrap" }}>
        ...
        {(location?.childLocations?.length ?? 0) === 0 && <Button key={`${location?.uuid}-actions-delete`} type="button" size="sm" className="submitButton clear-padding-margin" iconDescription={"Delete"} kind="ghost" renderIcon={TrashCan} onClick={(e) => onDeleteLocationClick(location.uuid)} />}
      </div> : ""
    })),
