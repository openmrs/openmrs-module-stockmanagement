...
import { StockOperationsTag, LIST_ID, StockOperationLinksTag } from './tagTypes'
...
const stockOperationsApi = api.injectEndpoints({
  endpoints: (build) => ({
    getStockOperations: build.query<PageableResult<StockOperationDTO>, StockOperationFilter>({
      query: (filter) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/stockmanagement/stockoperation${toQueryParams(filter)}`,
        method: 'GET'
      }),
      providesTags: (_result, _err, id) => {
        return [{ type: StockOperationsTag, id: LIST_ID }];
      },
      transformResponse: (response: PageableResult<StockOperationDTO>, meta, arg) => {
        //response?.results?.sort((a,b)=> a.operationNumber?.localeCompare(b.operationNumber ?? "") ?? 0);
        return response;
      }
    }),
    getStockOperationLinks: build.query<PageableResult<StockOperationLinkDTO>, string>({
      query: (filter) => ({
    ...
    updateStockOperation: build.mutation<StockOperationDTO, { model: StockOperationDTO, uuid: string }>({
      query: (stockOperation) => {
        return {
    ...
    executeStockOperationAction: build.mutation<StopOperationAction, StopOperationAction>({
      query: (stockOperation) => {
        return {
