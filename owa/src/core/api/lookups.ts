...
import { ConceptsTag, DrugsTag, LIST_ID, LocationsTag, LocationTagsTag, LocationWithIdTag, PartyTag, PatientsTag, RolesTag, StockOperationTypesTag, StockSourcesTag, UsersTag } from './tagTypes';
...
export const lookupsApi = api.injectEndpoints({
  endpoints: (build) => ({
    getLocations: build.query<PageableResult<OpenMRSLocation>, LocationFilterCriteria | null>({
      query: (filter) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/location${toQueryParams(filter, false)}`,
        method: 'GET'
      }),
    ...
    getLocationTags: build.query<PageableResult<OpenMRSLocationTag>, string | null>({
      query: (q) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/locationtag?v=default${q && q.length > 0 ? ("&q=" + encodeURIComponent(q)) : ""}`,
        method: 'GET'
      }),
      providesTags: (_result, _err, id) => {
    ...
    getStockOperationTypes: build.query<PageableResult<StockOperationType>, void>({
      query: () => ({
    ...
    getConcepts: build.query<PageableResult<Concept>, ConceptFilterCriteria>({
      query: (filter) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/concept${toQueryParams(filter)}`,
        method: 'GET'
      }),
      providesTags: (_result, _err, id) => {
    ...
    getPatients: build.query<PageableResult<Patient>, PatientFilterCriteria>({
      query: (filter) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/patient${toQueryParams(filter)}`,
        method: 'GET'
      }),
      providesTags: (_result, _err, id) => {
...
export const { useGetLocationsQuery,
  ...
  useLazyGetPatientsQuery } = lookupsApi;