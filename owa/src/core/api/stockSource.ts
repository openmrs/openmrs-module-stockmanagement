...
import { StockSourcesTag, LIST_ID } from './tagTypes'
...
const stockSourcesApi = api.injectEndpoints({
  endpoints: (build) => ({
    getStockSources: build.query<PageableResult<StockSource>, StockSourceFilter | null>({
      query: (filter) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/stockmanagement/stocksource${toQueryParams(filter)}`,
        method: 'GET'
      }),
    ...
    createOrUpdateStockSource: build.mutation<void, StockSource>({
      query: (stockSource) => {
        ...
        return {
          url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/stockmanagement/stocksource${isNew ? ("/" + stockSource.uuid) : ""}`,
          method: 'POST',
