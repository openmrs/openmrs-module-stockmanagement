...
> = async (args, api, extraOptions) => {
  ...
  if ((result.error || (result as any).data?.error)) {
    if ((result as any)?.error?.originalStatus === 401 || (result as any).status === 401 || result.error?.status === 401 /* Unauthorized  */) {
      api.dispatch(logout());
      retry.fail(result.error);
    } else if ((result as any)?.error?.originalStatus === 403 || (result as any).status === 403 || result.error?.status === 403  /* OpenMRS returns 403 */) {
      if ((result as any)?.error?.data) {
        let dataString = ((result as any)?.error?.data as any).toString();
        if (dataString.indexOf("<!doctype") >= 0 && dataString.indexOf('HTTP Status 403 – Forbidden') >= 0) {
          api.dispatch(logout());
          retry.fail(result.error);
        }
      }
...
export function toQueryParams<T extends ResourceFilterCriteria>(filterCriteria?: T | null, skipEmptyString = true): string {
