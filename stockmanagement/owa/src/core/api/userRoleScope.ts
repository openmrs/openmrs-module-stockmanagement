...
import { UserRoleScopesTag, LIST_ID } from './tagTypes'
...
const userRoleScopesApi = api.injectEndpoints({
  endpoints: (build) => ({
    getUserRoleScopes: build.query<PageableResult<UserRoleScope>, UserRoleScopeFilter>({
      query: (filter) => ({
        url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/stockmanagement/userrolescope${toQueryParams(filter)}`,
        method: 'GET'
      }),
    ...
    createOrUpdateUserRoleScope: build.mutation<void, UserRoleScope>({
      query: (userRoleScope) => {
        ...
        return {
          url: `${BASE_OPENMRS_APP_URL}ws/rest/v1/stockmanagement/userrolescope${isNew ? ("/" + userRoleScope.uuid) : ""}`,
          method: 'POST',
          body: userRoleScope
        }
      },
