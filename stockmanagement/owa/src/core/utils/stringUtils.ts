...
export function toErrorMessage(payload: any) {
    ...
    let errorMessage = payload?.error?.data?.error?.message ?? payload?.data?.error?.message ?? payload?.message ?? payload?.error ?? payload;
    let fieldErrors = payload?.error?.fieldErrors ?? payload?.data?.error?.fieldErrors;
    ...
    if (fieldErrors) {
        ...
        for (field in fieldErrors) {
            errors.push(fieldErrors[field]?.reduce((p: any, c: any, i: number) => {
