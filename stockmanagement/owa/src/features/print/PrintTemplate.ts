...
import { PRINT_LOGO_TEXT, PRINT_LOGO } from '../../config'
import { GetPrintLogo, PrintLogoData } from '../../core/utils/imageUtils'

export const GetPrintTemplate = (body: string, title: string | null | undefined, printOnLoad = false, closeAfterPrint = true) => {
    return `<html>
<head>
...
<body onload="${printOnLoad ? 'window.print()' : ''}" style='word-wrap:break-word' ${closeAfterPrint ? 'onafterprint="self.close()"' : ""}>
    ${body}
</body>
</html>`
}
...
<div class="logo" >
    ...
    ${PRINT_LOGO_TEXT ? `<span class='logo-text text'>${PRINT_LOGO_TEXT}</span>` : ""}
</div>    
    `: "";
}
